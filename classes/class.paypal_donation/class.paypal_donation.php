...
class paypal_donation {
	...
	private function set_defaults( $atts = '' ) {
		...
		if ( !is_numeric( $atts['total_dismissals'] ) || ( $atts['total_dismissals'] < 0 ) )
			$this->total_dismissals = 6;		// On the last appearance, the user will get the "Never Show Again" link
		else
...
$atts['paypal_message'] = '<p>The Multi Video Box plugin was created by <a href="http://www.nuttymango.com/" target="_blank">Scott Kustes at Nutty Mango</a>.  Donations help me support and maintain free plugins like this one.</p><p>If this plugin has improved how you get information to your readers, consider making a small donation to help keep the Wordpress community humming along.</p>';
$atts['paypal_item_name'] = 'Donation For Multi Video Box Plugin';
$atts['wp_option_name'] = 'ppd_mvob_options';
$atts['ajax_call'] = 'dismiss_ppd_mvob';

global $mvob_paypal;
$mvob_paypal = new paypal_donation( $atts );
?>